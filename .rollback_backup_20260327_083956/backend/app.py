...
def get_trip(trip_id):
    ...
    try:
        ...
        # Add global events linked to this trip_id (fallback path when AI posts to /events)
        global_trip_events = list(events_collection.find({"trip_id": trip_id}))
        for evt in global_trip_events:
            labels = evt.get("event_labels") or _extract_detection_labels(evt.get("detections", []))
            raw_type = str(evt.get("event_type") or "").strip()
            is_generic = (not raw_type) or (raw_type in {"DETECTION", "AI Detection", "AI_DETECTION", "Multiple"})
            primary = (
            ...
            consolidated_events.append({
                "timestamp": evt.get("timestamp") or evt.get("start_time"),
                "type": primary,
                "event_type": primary,
                "description": f"Risk: {evt.get('risk_level', 'UNKNOWN')} - Detections: {label_text}",
                "details": evt.get("reasons", []),
                "risk_level": evt.get("risk_level"),
                "detections": evt.get("detections", []),
                "event_labels": labels,
                "source": evt.get("source", "ai_engine"),
                "event_action": evt.get("event_action", "frame"),
            })
...
def add_ai_result(trip_id):
    ...
    try:
        ...
        event = {
            "event_label": str(payload.get("event_label") or payload.get("event_type") or "DETECTION"),
            "start_time": payload.get("start_time") or payload.get("timestamp") or datetime.utcnow().isoformat(),
            "end_time": payload.get("end_time") or payload.get("timestamp") or datetime.utcnow().isoformat(),
...
def get_events():
    ...
    try:
        ...
        for event in events:
            ...
            event["event_label"] = event.get("event_label") or event.get("event_type")
            event["start_time"] = to_ist_display(event.get("start_time"))
            event["end_time"] = to_ist_display(event.get("end_time"))
