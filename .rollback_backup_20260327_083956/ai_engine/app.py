...
_frame_persist_state: Dict[str, Dict[str, Any]] = {}
_frame_persist_lock = threading.Lock()
_active_events_state: Dict[str, Dict[str, Any]] = {}
_active_events_lock = threading.Lock()
_episode_event_types = tuple(
    t.strip()
    for t in os.getenv(
        "EPISODE_EVENT_TYPES",
        "drowsiness,yawning,distraction,driver_not_visible,driver_too_far_from_camera,camera_blocked",
    ).split(",")
    if t.strip()
)
...
_frame_persist_min_interval_s = float(os.getenv("FRAME_EVENT_PERSIST_INTERVAL_SECONDS", "3.0"))
MAX_EVENT_DURATION_S = float(os.getenv("MAX_EVENT_DURATION_SECONDS", "10.0"))
...
def _event_state_defaults() -> Dict[str, Any]:
    return {
        "active_event": None,
        "invisible_since": None,
        "stress_candidate_since": None,
        "prev_emotion": "neutral",
    }


def _compute_instant_events(
    *,
    cv_metrics: Dict[str, Any],
    thresholds_used: Dict[str, Any],
    current_emotion: str,
) -> Dict[str, bool]:
    ear = float(cv_metrics.get("ear", 1.0) or 1.0)
    mar = float(cv_metrics.get("mar", 0.0) or 0.0)
    yaw = abs(float(cv_metrics.get("yaw_angle", 0.0) or 0.0))
    face_detected = bool(cv_metrics.get("face_detected", False))

    ear_thr = float(thresholds_used.get("ear_drowsiness", EYE_AR_THRESH) or EYE_AR_THRESH)
    mar_thr = float(thresholds_used.get("mar_yawning", MOUTH_AR_THRESH) or MOUTH_AR_THRESH)
    head_thr = float(thresholds_used.get("head_turn", HEAD_TURN_THRESH) or HEAD_TURN_THRESH)

    neg = {"anger", "angry", "sad", "sadness", "fear", "disgust"}
    emo = str(current_emotion or "unknown").lower()

    return {
        "drowsiness": bool(face_detected and ear < ear_thr),
        "yawning": bool(face_detected and mar > mar_thr),
        "distraction": bool(face_detected and yaw > head_thr),
        "driver_not_visible": (not face_detected),
        "stress": emo in neg,
    }


def _pick_event_label(flags: Dict[str, bool]) -> Optional[str]:
    for label in ["driver_not_visible", "drowsiness", "distraction", "yawning", "stress"]:
        if bool(flags.get(label, False)):
            return label
    return None
...
def _update_event_state_machine(
    *,
    ...
    risk_level: str,
    instant_events: Dict[str, bool],
) -> List[Dict[str, Any]]:
    """Create end-event payloads only when the single active event transitions to inactive/changes."""
    det_types = {
        str(d.get("type") or "").strip().lower()
        for d in (detections or [])
        if isinstance(d, dict)
    }

    NEGATIVE = {"anger", "angry", "sad", "sadness", "fear", "disgust"}

    with _active_events_lock:
        state = _active_events_state.get(session_key)
        if state is None:
            state = _event_state_defaults()
            _active_events_state[session_key] = state

        active_event = state.get("active_event")

        invisible_since = state.get("invisible_since")
        if not face_detected:
            if invisible_since is None:
                invisible_since = now_epoch
        else:
            invisible_since = None
        state["invisible_since"] = invisible_since
        invisible_duration = (now_epoch - invisible_since) if invisible_since is not None else 0.0
        driver_not_visible = invisible_duration > 1.5

        prev_emotion = str(state.get("prev_emotion") or "neutral").lower()
        current_emotion_l = str(current_emotion or "unknown").lower()
        stress_candidate = (prev_emotion == "neutral") and (current_emotion_l in NEGATIVE)

        stress_candidate_since = state.get("stress_candidate_since")
        if stress_candidate:
            if stress_candidate_since is None:
                stress_candidate_since = now_epoch
        else:
            stress_candidate_since = None
        state["stress_candidate_since"] = stress_candidate_since
        stress_duration = (now_epoch - stress_candidate_since) if stress_candidate_since is not None else 0.0
        stress = stress_duration > 2.0

        if current_emotion_l and current_emotion_l != "unknown":
            state["prev_emotion"] = current_emotion_l

        events_detected = {
            "drowsiness": "drowsiness" in det_types,
            "yawning": "yawning" in det_types,
            "distraction": "distraction" in det_types,
            "driver_not_visible": driver_not_visible,
            "stress": stress,
        }

        confirmed_label = _pick_event_label(events_detected)
        ended_payloads: List[Dict[str, Any]] = []

        if active_event is None and confirmed_label:
            state["active_event"] = {
                "type": confirmed_label,
                "start_time": now_iso,
                "start_epoch": now_epoch,
            }
            print(f"EVENT START: {state['active_event']}")
            return ended_payloads

        if active_event is not None:
            active_type = str(active_event.get("type") or "")
            start_epoch = float(active_event.get("start_epoch") or now_epoch)
            force_end = (now_epoch - start_epoch) > MAX_EVENT_DURATION_S
            should_end = (confirmed_label is None) or (confirmed_label != active_type) or force_end

            if should_end:
                payload = {
                    "event_label": active_type,
                    "event_type": active_type,
                    "start_time": active_event.get("start_time") or now_iso,
                    "end_time": now_iso,
                    "risk_score": risk_score,
                    "risk_level": risk_level,
                    "emotion": current_emotion,
                    "confidence": float(emotion_conf or 0.0),
                    "source": "ai_engine",
                }
                ended_payloads.append(payload)
                print(f"EVENT END: {payload}")
...
def _build_frame_persistence_payload(
    *,
    session_key: str,
    trip_id: str,
    detections: List[Dict[str, Any]],
    ...
    """Emit low-rate frame events so meaningful detections are not silently dropped."""
    if not detections:
        return None

    labels = [str((d or {}).get("type") or "").strip() for d in detections if isinstance(d, dict)]
    labels = [l for l in labels if l]
    if not labels:
        return None

    primary = labels[0]
    label_sig = "|".join(sorted(set(labels)))
    now_ts = time.time()

    with _frame_persist_lock:
        state = _frame_persist_state.get(session_key, {})
        last_ts = float(state.get("last_ts", 0.0) or 0.0)
        last_sig = str(state.get("last_sig", ""))
        if (now_ts - last_ts) < _frame_persist_min_interval_s and label_sig == last_sig:
            return None
        _frame_persist_state[session_key] = {"last_ts": now_ts, "last_sig": label_sig}
...
def _compute_detection(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if image_data:
        ...
        if image is not None:
            ...
            # Keep identity/emotion lightweight in fast loop.
            ...
            detection_result["emotion"] = _empty_emotion_placeholder()
            detection_result["passenger_emotions"] = []
            ...
            # Passenger SOS gesture is produced by slow analytics and reused elsewhere.
            passenger_sos_result = {
                "type": "sos_gesture",
                "person": "passenger",
                "sos_detected": False,
                "sos_triggered": False,
...
def _get_active_trip_id() -> Optional[str]:
    """Return currently active trip id from backend, if available."""
    try:
        check_url = f"{BACKEND_BASE_URL.rstrip('/')}/trips/active-trip/live_map"
        check_req = Request(check_url, method="GET")
        with urlopen(check_req, timeout=5) as check_resp:
            check_data = json.loads(check_resp.read().decode("utf-8"))
            if bool(check_data.get("trip_active")):
                trip_id = str(check_data.get("trip_id") or "").strip()
...
def _post_result_to_backend(result_payload: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if not trip_id:
        fallback_trip_id = _get_active_trip_id()
        if fallback_trip_id:
            trip_id = fallback_trip_id
            result_payload["trip_id"] = fallback_trip_id
            is_active = True
        else:
            # No active trip and no trip_id provided: persist as background event.
            endpoint = f"{BACKEND_BASE_URL.rstrip('/')}/events"
            result_payload["trip_id"] = None
            ...
            try:
                ...
                if 200 <= int(status) < 300:
                    return True, "sent (events; no active trip)"
...
def _post_backend_async(
    *,
    ...
    for payload in (result_payloads or []):
        try:
            print(f"[AsyncPost] SENDING EVENT: {payload}")
...
def reset_trip_session_endpoint(trip_id: str) -> Any:
    ...
    _reset_episode_state(session_key=trip_id)
    with _frame_persist_lock:
        _frame_persist_state.pop(trip_id, None)
    with _active_events_lock:
        _active_events_state.pop(trip_id, None)
...
def analyze_frame() -> Any:
    ...
    # Slow analytics scheduling stays outside the fast detection path.
    session_hint = str(trip_id or "").strip() or f"driver:{_get_driver_id_from_trip(str(trip_id or ''))}"
    cached_analytics = _get_cached_analytics_state(session_hint)
    image_data = payload.get("image") or payload.get("frame")
    if image_data and (not bool(cached_analytics.get("running", False))):
        last_run_ts = float(cached_analytics.get("last_run_ts", 0.0) or 0.0)
        if (time.time() - last_run_ts) >= max(0.5, float(SLOW_ANALYTICS_INTERVAL_S)):
            image_for_slow = _decode_image(image_data)
            if image_for_slow is not None:
                _schedule_slow_analytics(
                    session_key=session_hint,
                    trip_id=str(trip_id or ""),
                    image_bgr=image_for_slow,
                    cv_metrics={},
                )
    ...
    resolved_trip_id = str(trip_id).strip() if trip_id else None
    trip_is_active = False
    if resolved_trip_id:
        trip_is_active = _is_trip_active(resolved_trip_id)
    if (not trip_is_active) and (not resolved_trip_id):
        resolved_trip_id = _get_active_trip_id()
        trip_is_active = bool(resolved_trip_id)
    print(f"[AnalyzeFrame] trip_active: {trip_is_active} | trip_id_in: {trip_id} | trip_id_resolved: {resolved_trip_id}")
    ...
    session_key = str(resolved_trip_id or f"driver:{_get_driver_id_from_trip(str(resolved_trip_id or ''))}")
    cached_analytics = _get_cached_analytics_state(session_key)
    emotion_result = cached_analytics.get("emotion_result", _empty_emotion_placeholder())
    ...
    warnings = alert_engine.get_warnings(
        trip_id=str(resolved_trip_id or "unknown_trip"),
    ...
    cached_driver_emotion = cached_analytics.get("driver_emotion") or {}
    driver_emotion_payload = {
        "driver_emotion": str(cached_driver_emotion.get("driver_emotion") or emo.get("dominant_emotion") or "unknown"),
        "confidence": float(cached_driver_emotion.get("confidence") or emo.get("confidence") or 0.0),
        "stress_level": str(cached_driver_emotion.get("stress_level") or emo.get("stress_level") or "LOW"),
    }

    face_detected = bool((detection_result.get("metrics") or {}).get("face_detected", False))
    current_emotion = str(emo.get("dominant_emotion") or "unknown")
    emotion_conf = float(emo.get("confidence") or 0.0)
    instant_events = _compute_instant_events(
        cv_metrics=detection_result.get("metrics", {}) or {},
        thresholds_used=(detection_result.get("personalization") or {}).get("thresholds_used", {}) or {},
        current_emotion=current_emotion,
    )
    instant_event = _pick_event_label(instant_events)
    print(f"DETECTIONS: {detection_result.get('detections', [])}")
    print(f"INSTANT EVENT: {instant_event}")

    ended_events = _update_event_state_machine(
        session_key=session_key,
        now_iso=ts,
        now_epoch=time.time(),
    ...
    for e in ended_events:
        p = dict(e)
        p["trip_id"] = resolved_trip_id if trip_is_active else None
        storage_payloads.append(p)
    ...
    if storage_payloads or sos_event_payload:
        for p in storage_payloads:
            print(f"SENDING EVENT: {p}")
        threading.Thread(
            target=_post_backend_async,
            kwargs={
                "result_payloads": storage_payloads,
                "sos_event_payload": sos_event_payload,
            },
            daemon=True,
            name=f"backend-post-{resolved_trip_id or 'unknown'}",
        ).start()

    fast_loop_ms = (time.perf_counter() - started_at) * 1000.0
    print(f"[AnalyzeFrame] fast_loop_ms={fast_loop_ms:.1f} detections={len(detection_result['detections'])} trip_id={resolved_trip_id}")
